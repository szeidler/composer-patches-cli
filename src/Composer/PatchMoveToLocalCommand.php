...
class PatchMoveToLocalCommand extends PatchBaseCommand
{
        ...
        $this->setName('patch-remote-to-local')
            ...
            ->setDefinition([
                new InputArgument(
                    ...
                    'The directory where the patches should be moved to.'
                ),
            ]);
    ...
    protected function execute(
        ...
        if ($this->getPatchType() === self::PATCHTYPE_ROOT) {
            ...
            throw new \Exception(
                'Composer patches seems to be not enabled. Please enable composer patches first.'
            );
        }

        $downloader = new HttpDownloader(
            $this->getIO(),
            $this->requireComposer()->getConfig()
        );
        ...
        foreach ($patches as $package => $packagePatches) {
            foreach ($packagePatches as $description => $url) {
                ...
                if (
                    ...
                    str_contains($url, 'merge_requests/')
                ) {
